...
class TestDataLoading:
    ...
    def test_data_loadin_for_fact_sales(self,connect_to_mysqldb):
        try:
            expected_query = """select sd.sales_id,sd.product_id,sd.store_id,sd.quantity,sd.sales_amount as total_sales,
                                sd.sale_date from sales_with_details as sd order by sd.sales_id,sd.product_id,sd.store_id"""
            actual_query = """select fs.sales_id,fs.product_id,fs.store_id,fs.quantity,round(fs.total_sales,0) as total_sales,fs.sale_date 
    ...
    def test_data_loading_for_inventory_levl_by_stores(self,connect_to_mysqldb):
        try:
            expected_query = """select store_id,astotal_inventory from aggregated_inventory_level"""
            actual_query = """select store_id,astotal_inventory from inventory_levels_by_store"""
            test_case_name = inspect.currentframe().f_code.co_name
            self.validation_utility.execute_validation(
                validation_type='DB_TO_DB'
                , test_case_name=test_case_name
